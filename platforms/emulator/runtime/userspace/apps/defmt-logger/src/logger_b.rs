//! Logger B - Alternative defmt logger implementation
...
/// Write encoded bytes to the console output - Logger B format
///
/// This writes the defmt-encoded bytes to the Tock console, prefixed with [DEFMT-B: marker
fn write_bytes(bytes: &[u8]) {
    #[cfg(target_arch = "riscv32")]
    {
        let mut console = Console::<libtock_runtime::TockSyscalls>::writer();
        // Logger B uses [DEFMT-B: prefix
        let _ = write!(console, "[DEFMT-B:");
        for byte in bytes {
...
unsafe impl defmt::Logger for LoggerB {
    fn acquire() {
        ENCODER.acquire();
    }
